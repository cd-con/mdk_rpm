    ...
    public partial class MainWindow : Window
    {
        ...
        private void Solve_Click(object _, RoutedEventArgs __)
        {
            ...
            if (storage.Values != null)
            {
                ...
                if(storage.Values.GetLength(0) < 2) {
                    MessageBox.Show("Ачё)");
                    return;
                }
                (int, int)[] solution = new (int, int)[1];
                try
                {
                    solution = Enumerable.Range(1, storage.Values.GetLength(0) + 1).Where(x => x % 2 == 0).Select(x => (x, storage.Values.GetColumn(x - 1).Sum())).ToArray();
                }
                catch (IndexOutOfRangeException e) { }

                foreach ((int, int) sum in solution)
                    resultBox.Text += sum.Item1 + " сумма элементов = " + sum.Item2 + "\n";
            }
            else
            {
